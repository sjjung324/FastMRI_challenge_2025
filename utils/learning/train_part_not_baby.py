...
from utils.model.not_baby_unet import Unet

def train_epoch(args, epoch, model, data_loader, optimizer, loss_type, scheduler=None):
    model.train()
...
def train(args):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=2, gamma=0.5)  # StepLR: every 2 epochs, lr *= 0.5 (you can adjust step_size/gamma as needed)
