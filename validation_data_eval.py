...
def forward(args):
    ...
    if args.modality == 'all':
        test_parts = ['knee', 'brain']
    else:
        test_parts = [f'{args.modality}']
    ...
    with torch.no_grad():
        for fname in leaderboard_data:
                fname = fname.replace(str(args.leaderboard_data_path), '')
                if args.modality != 'all' and args.modality not in fname:
                    continue
                l_fname = Path(str(args.leaderboard_data_path) + fname)
                y_fname = Path(str(args.your_data_path) + fname)
...
if __name__ == '__main__':
    ...
    # acc4
    args.leaderboard_data_path = args.path_leaderboard_data / 'image'
    args.your_data_path = args.path_your_data
    SSIM_loss, L1_loss = forward(args)
    L1_loss = L1_loss * 10**4

    print("Leaderboard SSIM (acc4): {:.4f}".format(SSIM_loss))
    print("Leaderboard L1   (acc4): {:.6f}".format(L1_loss))
